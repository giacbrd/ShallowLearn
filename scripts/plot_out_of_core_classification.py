...
class ReutersParser(html_parser.HTMLParser):
    ...
    def parse(self, fd):
        self.docs = []
        for chunk in fd:
            self.feed(to_unicode(chunk, encoding=self.encoding))
...
X_test_tokenized = [list(tokenize(doc)) for doc in X_test_text]
...
for i, (X_train_text, y_train) in enumerate(minibatch_iterators):
    ...
    X_train_tokenized = [list(tokenize(doc)) for doc in X_train_text]

    for cls_name, cls in partial_fit_classifiers.items():
        tick = time.time()
        # update estimator with examples in the current mini-batch
        if isinstance(cls, GensimFastText):
            cls.partial_fit(X_train_tokenized, y_train)
        else:
        ...
        tick = time.time()
        if isinstance(cls, GensimFastText):
            cls_stats[cls_name]['accuracy'] = cls.score(X_test_tokenized, y_test)
        else:
